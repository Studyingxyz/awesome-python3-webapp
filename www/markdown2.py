...
#Dev Notes:
#- Python's regex syntax doesn't have '\z',so I'm using '\Z'.
#I'm not yet sure if there implications with this. Compare 'pydoc sre' and 'perldoc perlre'.
...
#Use `bytes` for byte strings and `unicode` for unicode strings( str in python3)
...
g_escape_table=dict([(ch,-hash_text(ch))
	for ch in '\\`*{}[]()>#+-.!'])
...
class Markdown(object):
	...
	#Used to track when we're inside ordered or unordered list
	#(see_ProcessListItems() for details):
	list_level=0
	...
	def __init__(self,html4tags=False,tab_width=4,safe_mode=None,
		...
		#For compatibility with earlier markdown2.py and with markdown.py's safe_mode being a boolean,
		...
		#Message and building the "extras" info.
		...
		self._outdent_re=re.compile(r'^(t|[]{1,%d})' % tab_width,re.M)
	...
	def convert(self,text):	
		...
		if self.use_file_vars:
			...
			if "markdown-extras" in emacs-vars:
				splitter=re.compile("[ ,]+")
	...
	def _get_emacs_vars(self,text):
		...
		for var,val in  list(emacs_vars.items()):
			if len(val)>1 and (var.startswith('"') and val.endswith('"')
	...
	def _detab_sub(self,match):
		gl=match.group(1)
		return gl+('  '*(self.tab_width-len(gl) % self.tab_width))
	...
	def _hash_html_blocks(self,text,raw=False):
		...
		#Pass `raw`value into our calls to self._hash_html_blocks_sub.
    ...
    def _run_span_gamut(self, text):
		...
		if "link_patterns" in self.extras:
    ...
    def _escape_special_chars(self, text):
		...
		for token in self._sorta_html_tokenize_re.split(text):
			...
			is_html_markup=not in is_html_markup
		return ''.join(escaped)
	...
	def _hash_html_spans(self,text):
		...
		def _is_auto_link(s):
			if ':'in s and self._auto_link_re.compile(s):
				return True
			elif '@'in s and self._auto_email_link_re.compile(s):
				return True
			return False
	...
	def _do_links(self,text):
		...
		while True:# Handle the next link.
			...
			if text[p]=='(': #attempt at perf impovement
	...
	def header_id_from_text(self,text,prefix,n):
		...
		if header-id in self._count_from_header_id:
	...
	def _h_sub(self,match):
		...
		demote_headers=self.extras.get("demote_headers")
		if demote_headers:
		...
		html=self._run_span_gamut(header_id)
	...
	def _do_fenced_code_blocks(self,text):
		"""Process ```-fenced unindented code blocks('fenced-code-block'extra)."""
	...
	def _encode_code(self,text):
		...
		self._escape_table[text]=hahsed
		return hashed
		
	_strong_re=re.compile(r"(\*\*|__)(?=\S)(.+?[*_]*)(?<=\S)\1",re.S)
	_em_re=re.compile(r"(\*|_)(?=\S)(.+?)(?<=\S)\1",re.S)
	_code_frienfly_strong_re=re.compile(r"\*\*(?=\S)(.+?[*_]*)(?<=\S)\*\*",re.S)
	_code_frienfly_em_re=re.compile(r"\*(?=\S)(.+?)(?<=\S)\*",re.S)
	def _do_italics_and_bold(self,text):
		#<strong> must go frist
		if "code-friendly" in self.extras:
			text=self._code_frienfly_strong_re.sub(r"<strong>\1</strong>",text)
			text=self._code_frienfly_em_re.sub(r"<em>\1</em>",text)
		else:
...
class _memoized(object):
	...
	def __call__(self,^args):
		try:
			...
			self.cachep[args]=value=self.func(*args)
			return value
		except TypeError:
...
def _xml_oneliner_re_from_tab_width(tab_width):
	"""Standlone XML processing instruction regex."""
	return re.compile(r"""
        (?:
